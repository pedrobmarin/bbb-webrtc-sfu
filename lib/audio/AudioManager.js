...
const maxAudioConnections = config.get('maxAudioConnections');
  ...
  async _onMessage(message) {
    ...
    switch (message.id) {
      case 'start':
        ...
        let connectedUsers = this._numberOfConnectedUsers();
        if (connectedUsers >= maxAudioConnections) {
          Logger.warn('Server reached maximum audio connections limit', connectedUsers);
          return notifyStartError('Server reached maximum audio connections limit');
        }

        if (session == null) {
          session = new Audio(this._bbbGW, voiceBridge);
          session.once(C.MEDIA_STOPPED, this._stopSession.bind(this));
          this._sessions[sessionId] = {}
          this._sessions[sessionId] = session;
          try {
            await session.upstartSourceAudio(message.sdpOffer, message.callerName);
          } catch (err) {
