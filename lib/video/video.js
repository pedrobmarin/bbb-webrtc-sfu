...
module.exports = class Video extends BaseProvider {
  constructor(bbbGW, _meetingId, _id, _shared, _connectionId, mcs, voiceBridge, bbbUserId, bbbUserName, strategy) {
    super(bbbGW);
  ...
  getSource(id) {
    if (this.strategy === 'VOICE_SWITCHING') return 'default';
    else return sources[id];
  }
    ...
    try {
      ...
      switch (this.strategy) {
        case 'VOICE_SWITCHING':
          Logger.info(LOG_PREFIX, 'User', this.userId, 'will be voice switching at', this.voiceBridge);
          options = Object.assign(options, { strategy: this.strategy });
          Logger.trace(LOG_PREFIX, 'Using', C.WEBRTC, 'and', options);
          response = await this.mcs.publish(this.userId, this.voiceBridge, C.WEBRTC, options);
          break;
        default:
          response = await this.mcs.subscribe(this.userId, this._source(this.id), C.WEBRTC, options);
      }
